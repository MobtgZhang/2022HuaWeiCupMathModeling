...
class TimeRNNModel(torch.nn.Module):
    def __init__(self,val_seq_len):
        ...
        self.gru_time1 = torch.nn.GRU(val_seq_len,val_seq_len,num_layers=1,batch_first=True,bidirectional=True)
        ...
        self.gru_time2 = torch.nn.GRU(val_seq_len,val_seq_len,num_layers=1,batch_first=True)
    def forward(self,x):
...
def main():
    ...
    if not os.path.exists(model_path):
        ...
        model = TimeRNNModel(fea_dim).to(device)
        ...
        for epoch in range(args.train_times):
            ...
            for idx,(prev,post) in enumerate(train_loader):
                ...
                loss.backward()
                optimizer.step()
            all_loss = all_loss/len(train_loader)
            re_dict = evaluate(model,test_loader,loss_fn,device)
