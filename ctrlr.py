...
class Ctrlr(torch.nn.Module):
	...
	def __init__(self, input_sz, output_sz, N, M, ctrlr_sz, ctrlr_layers, heads):
		...
		self.rnn = torch.nn.GRU(input_size=self.ctrlr_input_sz, hidden_size=ctrlr_sz, num_layers=ctrlr_layers)

		# the hidden variable will be optimized by optimizer
		self.lstm_h_bias = torch.nn.Parameter(torch.randn(ctrlr_layers, 1, ctrlr_sz) * 0.05)
		# self.lstm_c_bias = nn.Parameter(torch.randn(ctrlr_layers, 1, ctrlr_sz) * 0.05)

		# output of controller to output of NTM
		# [128 + head_num*20] => [8]
		self.o2o = torch.nn.Linear(self.ctrlr_sz + self.num_read_heads * self.M, output_sz)
	...
	def reset_parameters(self):
		...
		torch.nn.init.xavier_uniform_(self.o2o.weight, gain=1)
		torch.nn.init.normal_(self.o2o.bias, std=0.01)
